...
({
    ...
    startDeleteProcess : function(cmp, event, helper){
        ...
        if(confirm && cmpId == cmp.get("v.cmpId")) {
            cmp.set("v.processing", true);
            var action = cmp.get("c.deleteEvent");
            action.setParams({
                "eventId": cmp.get("v.recordId")
            });
            action.setCallback(this, function (response) {
                ...
                if (state === "SUCCESS") {
                    ...
                    if (success) {
                        ...
                        evt.setParams({
                            componentDef : "c:AACREventsDashboard",
                        });
                        evt.fire();
    ...
    openModal : function(cmp, event, helper){
        cmp.find('deleteConfirmModal').set("v.isOpen", true);
    }
})