...
({
    closeModal : function (cmp,batchId){
        let cmpEvent = cmp.getEvent('sponsorEvent');
        cmpEvent.setParams({
            batchId : batchId
        })
        cmpEvent.fire();
    ...
    validatePassword: function (cmp) {
        let inputField = cmp.find('pwValue');
        ...
        if(validity.valid){
            if(cmp.get('v.passwordStrength') == 'Valid'){
               return true;
            }else{
                return false;
            }
